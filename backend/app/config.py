...
class Settings(BaseSettings):
    ...
    # Chrome's own sandbox, in a container. "auto" tries to launch with it
    # and falls back to --no-sandbox if Chrome will not start; "on" never adds
    # the flag; "off" always does. It was "off" until the smoke workflow showed
    # Chrome starting sandboxed as pwuser on a current Docker; older kernels and
    # stricter seccomp profiles still need the flag, which is what "auto" is
    # for. See `browser.resolve_channel`, which makes the decision by
    # launching, and logs it.
    container_sandbox: str = "auto"
